...
Public endpoints via Cloudflare Tunnel (see [`endpoints.yaml`](endpoints.yaml) for full config):

| Service | URL | Use for |
|---------|-----|---------|
| **vLLM** (direct) | `https://ada-sporting-marble-mike.trycloudflare.com` | Direct model access, full model name |
| **LiteLLM** (proxy) | `https://writer-annotation-management-modeling.trycloudflare.com` | Model aliases (`haiku`/`sonnet`/`opus`) |
| **Open WebUI** | `https://portion-florida-explicit-conviction.trycloudflare.com` | Chat interface |
| **Ray Dashboard** | `https://webshots-tomorrow-pension-dom.trycloudflare.com` | GPU/cluster monitoring |

> Tunnel URLs are generated by trycloudflare.com and change on instance restart. Check `endpoints.yaml` for current URLs.

### Cursor / Continue / Claw

```
Base URL:  https://writer-annotation-management-modeling.trycloudflare.com/v1
API Key:   none
Model:     haiku  (or sonnet, opus — all route to M2.5)
```
...
client = OpenAI(
    base_url="https://ada-sporting-marble-mike.trycloudflare.com/v1",
    api_key="none"
)
...
```bash
curl https://ada-sporting-marble-mike.trycloudflare.com/v1/chat/completions \
